...
iDCOMTestSrv.idl
    ...
    此文件将由 MIDL 编译器处理，用于生成：
        ...
        封送处理代码                                 （iDCOMTestSrv_p.c 和 dlldata.c）

iDCOMTestSrv.h
    此文件包含 iDCOMTestSrv.idl 中定义的项目的 C++ 接口定义和 GUID 声明。它将在编译过程中由 MIDL 重新生成。
...
StdAfx.h, StdAfx.cpp
    这些文件用于生成名为 iDCOMTestSrv.pch 的预编译头 (PCH) 文件和名为 StdAfx.obj 的预编译类型文件。
...
iDCOMTestSrvps.vcxproj
	...
	此过程生成 dlldata.c、iDCOMTestSrv_i.c 和 iDCOMTestSrv_p.c，这些是生成代理/存根 (stub) DLL 所必需的。
