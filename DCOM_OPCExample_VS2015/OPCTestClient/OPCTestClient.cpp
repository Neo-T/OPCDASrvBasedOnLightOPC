...
//* 需要注意，由于我们编译的是64位版本，OPC库使用的OPCHANDLE则是DWORD类型，其经常作为保存指针地址的变量类型来用，64位机器的指针地址
//* 也是64位的，这样就会缺失高32位数值，导致程序运行故障，所以一定注意一旦使用带有OPCHANDLER参数的函数传递地址时，要手动将函数参数类
//* 型改为64位，也就是不再使用OPCHANDLER作为参数类型，显式的采用LONG64类型
